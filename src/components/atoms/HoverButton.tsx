...
const HoverButton: React.FC<HoverButtonProps> = ({
    ...
    const baseStyles = `
        ...
        ${compactOnMobile ? 'p-2 sm:px-8 sm:py-2' : 'px-6 py-2'}
        ${className}
    `;

    // Arrow SVG definition to avoid repetition
    ...
    const content = (
        <>
            {/* The Background Circle Animation */}
            ...
            <span className="absolute left-4 opacity-0 -translate-x-4 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300">
                <ArrowIcon className="" />
            </span>
            ...
            <div className="flex items-center gap-2 transition-transform duration-300 group-hover:translate-x-3">
                ...
                {/* Text: Hidden on mobile if compactOnMobile is true */}
            ...
            <span className="absolute right-4 opacity-100 translate-x-0 group-hover:opacity-0 group-hover:translate-x-4 transition-all duration-300">
               <ArrowIcon className="" />
            </span>
        </>
    );
